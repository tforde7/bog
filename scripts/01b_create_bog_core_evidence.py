...
Run from the QGIS Python Console. This correction writes a normal GeoPackage
path, after removing only any incomplete output created by the earlier script.
"""
...
MALFORMED_OUTPUT = OUTPUT.parent / "01_bog_core_evidence.gpkg|layername=bog_core_evidence_itm.gpkg"
...
# Replace only the output created by this step and the malformed retry artifact.
for path in (OUTPUT, Path(f"{OUTPUT}-wal"), Path(f"{OUTPUT}-shm"), MALFORMED_OUTPUT):
    if path.exists():
        path.unlink()
